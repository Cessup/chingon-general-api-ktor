...
fun Route.starterRoutes(
    ...
    route("eatable/meals/starter") {
        ...
        */
        post("/new") {
            val beer = call.receive<Starter>()
            val result: Boolean = newMealUseCase.execute(beer) { insertStarter(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getStarters() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteStarter(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Starter deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Starter not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Starter>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateStarter(it) }
            if(result){
