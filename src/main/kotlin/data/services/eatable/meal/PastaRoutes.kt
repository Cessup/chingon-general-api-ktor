...
fun Route.pastaRoutes(
    ...
    route("eatable/meals/pasta") {
        ...
        */
        post("/new") {
            val beer = call.receive<Pasta>()
            val result: Boolean = newMealUseCase.execute(beer) { insertPasta(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getPastas() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deletePasta(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Pasta deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Pasta not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Pasta>()
            val result: Boolean = updateMealUseCase.execute(beer) { updatePasta(it) }
            if(result){
