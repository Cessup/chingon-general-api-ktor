...
fun Route.soupRoutes(
    ...
    route("eatable/meals/soup") {
        ...
        */
        post("/new") {
            val beer = call.receive<Soup>()
            val result: Boolean = newMealUseCase.execute(beer) { insertSoup(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getSoups() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteSoup(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Soup deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Soup not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Soup>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateSoup(it) }
            if(result){
