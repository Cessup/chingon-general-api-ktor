...
fun Route.riceRoutes(
    ...
    route("eatable/meals/rice") {
        ...
        */
        post("/new") {
            val beer = call.receive<Rice>()
            val result: Boolean = newMealUseCase.execute(beer) { insertRice(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getRices() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteRice(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Rice deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Rice not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Rice>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateRice(it) }
            if(result){
