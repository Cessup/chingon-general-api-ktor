...
fun Route.breadRoutes(
    ...
    route("eatable/meals/bread") {
        ...
        */
        post("/new") {
            val beer = call.receive<Bread>()
            val result: Boolean = newMealUseCase.execute(beer) { insertBread(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getBreads() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteBread(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Bread deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Bread not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Bread>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateBread(it) }
            if(result){
