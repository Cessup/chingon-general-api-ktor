...
fun Route.dessertRoutes(
    ...
    route("eatable/meals/dessert") {
        ...
        */
        post("/new") {
            val beer = call.receive<Dessert>()
            val result: Boolean = newMealUseCase.execute(beer) { insertDessert(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getDesserts() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteDessert(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Dessert deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Dessert not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Dessert>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateDessert(it) }
            if(result){
