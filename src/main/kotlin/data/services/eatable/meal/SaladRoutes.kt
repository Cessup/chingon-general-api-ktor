...
fun Route.saladRoutes(
    ...
    route("eatable/meals/salad") {
        ...
        */
        post("/new") {
            val beer = call.receive<Salad>()
            val result: Boolean = newMealUseCase.execute(beer) { insertSalad(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getSalads() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteSalad(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Salad deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Salad not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Salad>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateSalad(it) }
            if(result){
