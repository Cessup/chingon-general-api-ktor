...
fun Route.mainRoutes(
    ...
    route("eatable/meals/main") {
        ...
        */
        post("/new") {
            val beer = call.receive<Main>()
            val result: Boolean = newMealUseCase.execute(beer) { insertMain(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getMains() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteMain(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Main deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Main not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Main>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateMain(it) }
            if(result){
