...
fun Route.bakeryRoutes(
    ...
    route("eatable/meals/bakery") {
        ...
        */
        post("/new") {
            val beer = call.receive<Bakery>()
            val result: Boolean = newMealUseCase.execute(beer) { insertBakery(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getBakeries() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteBakery(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Bakery deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Bakery not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Bakery>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateBakery(it) }
            if(result){
