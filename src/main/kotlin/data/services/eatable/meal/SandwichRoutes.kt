...
fun Route.sandwichRoutes(
    ...
    route("eatable/meals/sandwich") {
        ...
        */
        post("/new") {
            val beer = call.receive<Sandwich>()
            val result: Boolean = newMealUseCase.execute(beer) { insertSandwich(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getSandwiches() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteSandwich(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Sandwich deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Sandwich not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Sandwich>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateSandwich(it) }
            if(result){
