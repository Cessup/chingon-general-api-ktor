...
fun Route.bakedRoutes(
    ...
    route("eatable/meals/baked") {
        ...
        */
        post("/new") {
            val beer = call.receive<Baked>()
            val result: Boolean = newMealUseCase.execute(beer) { insertBaked(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getMealsUseCase.execute{ getBakeds() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteBaked(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Baked deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Baked not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Baked>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateBaked(it) }
            if(result){
