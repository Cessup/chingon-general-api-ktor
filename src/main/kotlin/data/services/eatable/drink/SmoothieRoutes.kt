 ...
 *
 * @constructor [NewDrinkUseCase] the NewSmoothieUseCase is a use case to make a new products
 * @constructor [UpdateDrinkUseCase] the UpdateSmoothieUseCase is a use case to update a smoothie
 * @constructor [DeleteDrinkUseCase] the DeleteSmoothieUseCase is a use case to delete a smoothie
 * @constructor [GetDrinksUseCase] the GetSmoothiesUseCase is a use case to give a list of smoothies
 *
 * @author
 *     Cessup
 * @since 1.0
 */
fun Route.smoothieRoutes(
    ...
    route("eatable/drinks/smoothie") {
        ...
        */
        post("/new") {
            val smoothie = call.receive<Smoothie>()
            val result: Boolean = newDrinkUseCase.execute(smoothie) { insertSmoothie(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getDrinksUseCase.execute{ getSmoothies() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteDrinkUseCase.execute(id){ deleteSmoothie(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Smoothie deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Smoothie not found")
            }
        }
       ...
       */
        put("/update") {
            val smoothie = call.receive<Smoothie>()
            val result: Boolean = updateDrinkUseCase.execute(smoothie) { updateSmoothie(it) }
            if(result){
