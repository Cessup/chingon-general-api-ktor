 ...
 *
 * @constructor [NewDrinkUseCase] the NewCocktailUseCase is a use case to make a new products
 * @constructor [UpdateDrinkUseCase] the UpdateCocktailUseCase is a use case to update a cocktail
 * @constructor [DeleteDrinkUseCase] the DeleteCocktailUseCase is a use case to delete a cocktail
 * @constructor [GetDrinksUseCase] the GetCocktailsUseCase is a use case to give a list of cocktails
 *
 * @author
 *     Cessup
 * @since 1.0
 */
fun Route.cocktailRoutes(
    ...
    route("eatable/drinks/cocktail") {
        ...
        */
        post("/new") {
            val cocktail = call.receive<Cocktail>()
            val result: Boolean = newDrinkUseCase.execute(cocktail) { insertCocktail(it) }
            if (result) {
        ...
        */
        get("/get") {
            val drinks = getDrinksUseCase.execute{ getCocktails() }
            if (drinks != null) {
                call.respond(drinks)
            } else {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteDrinkUseCase.execute(id){ deleteCocktail(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Cocktail deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Cocktail not found")
            }
        }
       ...
       */
        put("/update") {
            val cocktail = call.receive<Cocktail>()
            val result: Boolean = updateDrinkUseCase.execute(cocktail) { updateCocktail(it) }
            if(result){
