...
fun Route.mealRoutes(
    ...
    route("eatable/meals/meal") {
        ...
        */
        post("/new") {
            val beer = call.receive<Meal>()
            val result: Boolean = newMealUseCase.execute(beer) { insertMeal(it) }
            if (result) {
        ...
        */
        delete("/delete") {
            ...
            val deleted = deleteMealUseCase.execute(id){ deleteMeal(id)}
            if (deleted) {
                call.respond(HttpStatusCode.OK, "Meal deleted successfully")
            } else {
                call.respond(HttpStatusCode.NotFound, "Meal not found")
            }
        }
       ...
       */
        put("/update") {
            val beer = call.receive<Meal>()
            val result: Boolean = updateMealUseCase.execute(beer) { updateMeal(it) }
            if(result){
