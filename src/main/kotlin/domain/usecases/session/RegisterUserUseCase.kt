 ...
 *
 * @constructor Receiver a [UserRepository] and a [Security] object because use it to origin data.
 * @author
 *     Cessup
 * @since 1.0
 */
class RegisterUserUseCase(val repository: UserRepository, val security: Security){
    ...
    suspend fun execute(registerRequest: RegisterUserRequest): Boolean {
        repository.findByEmail(registerRequest.email)?.let { throw IllegalArgumentException("Email already in use") }
        repository.findByPhone(registerRequest.phone)?.let { throw IllegalArgumentException("Phone already in use") }
        repository.findByPhone(registerRequest.nickname)?.let { throw IllegalArgumentException("NickName already in use") }
        ...
        val resultNewUser = repository.insertUser(user)
        //Find the role
        val list = repository.findRoles()
        ...
        val resultTypeUser = repository.insertType(type)
